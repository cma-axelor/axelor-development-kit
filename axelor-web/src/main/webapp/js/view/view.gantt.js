  ...
  function GanttViewCtrl($scope, $element) {
    ...
    $scope.fetchItems = function(callback) {
      ...
      if(schema.taskSequence) {
        opts['sortBy'] = [schema.taskSequence];
      }
    ...
    function link(scope, element, attrs, controller) {
      ...
      function setScaleConfig(value){
       ...
       function setChildTaskDisplay() {
        ...
        gantt.templates.grid_row_class = gantt.templates.task_class=function(start, end, task){
          ...
          if (task.planned_end && task.type != gantt.config.types.milestone) {
            css.push("has-baseline");
           ...
           renderer:{
             render: function(task) {
                if(task.planned_start && task.planned_end) {
                  ...
                  el.style.background = byId(gantt.serverList("users"), task.user_id, 'backgroundColor') || '#ff0000';
                  return el;
                }
                return false;
             },
      ...
      function updateData(task, rec){
        ...
        if(schema.taskUser && rec[schema.taskUser]) {
          ...
          if(!byId(gantt.serverList("users"), task.user_id,'label')) {
        ...
        if(schema.isMileStone && rec[schema.isMileStone]){
